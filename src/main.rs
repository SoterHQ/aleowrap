...
fn main() {
    ...
    println!("\n\nSend Messaging");
    let start_time = Instant::now();
    let broadcast = Some("https://vm.aleo.org/api/testnet3/transaction/broadcast".to_string());
    let dry_run = true;
    ...
    let execution_time = end_time - start_time;
    println!(
        "Send Message usetime:{:?} transaction: {}",
        execution_time,
        handle_res.unwrap()
    );
}
...
mod tests {
    ...
    use snarkvm::{
        prelude::{transaction::Transaction, Locator, Testnet3},
        utilities::FromBytes,
    };
